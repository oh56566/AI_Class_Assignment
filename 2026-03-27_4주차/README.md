...
| DecisionTree random_state | 42 | 42 |
| RandomForest random_state | 42 | 42 |
...
상관관계 기반 Feature 선택은 Logistic Regression에서 소폭 성능 저하를 가져왔지만, Decision Tree와 Random Forest에서는 유의미한 성능 향상을 이끌어냈다. 이를 통해 트리 계열 모델이 불필요한 Feature에 더 민감하게 반응한다는 점을 실험적으로 확인했다. 데이터 탐색 → 전처리 → Feature 선택 → 다중 모델 비교의 전체 워크플로우를 직접 수행한 실습이었다.
