...
성별이 생존에 가장 큰 영향을 미쳤다 (상관계수 -0.54) "여성과 어린이 먼저" 라는 당시 구조 원칙이 데이터로 실증된다. 여성 생존율이 남성보다 상당히 높다.
...
나이는 생각보다 영향이 적었다 (상관계수 -0.06) 상대적으로 적은 데이터에 나이 결측치가 177개(20%)나 되어 신뢰도가 낮다.
