...
| 스케일링 (y) | StandardScaler — 회귀 타겟에만 적용, 역변환으로 실제 단위 확인 | BC_reg, Diabetes_reg |
...
| Epochs | 50 | 30 | 100 | 100 | 50 | 100 |
| 특이사항 | Dropout(0.3) | X·y 모두 스케일링 | class_weight 불균형 보정 | 파생 피처 2개 추가 | get_dummies 원핫 | 스케일링 X만 적용 |
...
알코올 함량(Alcohol)과의 상관관계를 기반으로 7개 피처를 선택하였다(Proline, Color.int, Acl, Phenols, Mg, Flavanoids, Ash). X에만 StandardScaler를 적용하였으며, y(Alcohol)는 스케일링하지 않아 MSE가 실제 단위로 출력된다.
...
- **Data Leakage 방지**: `train_test_split` 이후 `scaler.fit_transform(X_train)` → `scaler.transform(X_test)` 순서를 모든 실습에서 일관되게 적용하였다.
- **결측값의 정확한 식별**: Diabetes 데이터에서 NaN이 아닌 0값이 의학적 결측임을 파악하고, `replace(0, NA)` 후 중앙값으로 대체하였다.
- **회귀 타겟 스케일링**: BC_reg와 Diabetes_reg에서 y도 StandardScaler를 적용하고, 결과 해석 시 역변환하여 실제 단위의 RMSE를 도출하였다.
- **문제 유형에 맞는 출력층 설계**: 이진분류(sigmoid+BCE), 다중분류(softmax+CCE), 회귀(선형+MSE)를 각각 정확히 적용하였다.
- **클래스 불균형 처리**: Diabetes 분류에서 `class_weight='balanced'`를 적용하여 소수 클래스(당뇨)의 예측 성능을 개선하였다.

### 개선 사항

- **Diabetes_reg 성능 한계**: BMI를 설명하는 피처의 상관관계가 전반적으로 낮아 R² ≈ 0.18에 그쳤다. 체중, 신장 등 직접적인 피처가 데이터셋에 없는 구조적 한계이다.
- **Wine_reg y 스케일링 누락**: 알코올 함량(y)에 StandardScaler를 적용하지 않아 MSE가 실제 단위로 계산되었다. BC_reg처럼 y도 스케일링 후 역변환하면 학습 안정성이 향상될 수 있다.
- **과적합 모니터링 부재**: Train-Val Loss 차이가 큰 경우(Diabetes_reg) Early Stopping 또는 Dropout 강화 등의 대응책을 추가하면 일반화 성능이 개선될 수 있다.

### 결론

이번 실습을 통해 딥러닝 모델의 성능은 알고리즘 구조보다 데이터 품질과 전처리의 영향이 크다는 것을 확인하였다. Breast Cancer와 Wine처럼 피처-타겟 간 상관관계가 명확한 데이터셋에서는 높은 성능(97~100%)을 달성하였으나, Diabetes처럼 피처 설명력이 낮고 결측률이 높은 데이터셋에서는 전처리를 충실히 수행하더라도 성능에 한계가 존재함을 이해하였다. 또한 문제 유형(이진분류/다중분류/회귀)에 따른 출력층 활성함수와 손실함수 조합을 명확히 구분하여 적용하는 역량을 갖추었다.
